...
INSTALL_CMD = "pip install pandas numpy scikit-learn tqdm torch transformers accelerate peft"
...
def make_epoch_train_loader(
    train_dataset: TensorDataset,
    batch_size: int,
    pin_memory: bool,
    epoch: int,
) -> DataLoader:
    # Epoch-dependent seeding makes shuffle order reproducible across resumed runs.
    train_generator = torch.Generator().manual_seed(SEED + epoch)
    return DataLoader(
        train_dataset,
...
def build_optimizer(model: nn.Module, learning_rate: float, adam_eps: float) -> AdamW:
...
def build_lora_model() -> nn.Module:
...
def save_last_checkpoint(
    ...
    best_epoch: int,
    best_epoch_f1: float,
    best_epoch_val_probs: np.ndarray | None,
    best_epoch_val_labels: np.ndarray | None,
    ...
    torch.save(
        {
            ...
            "best_epoch_val_probs": None if best_epoch_val_probs is None else best_epoch_val_probs.astype(np.float32),
            "best_epoch_val_labels": None
            if best_epoch_val_labels is None
            else best_epoch_val_labels.astype(np.int64),
...
def load_lora_model_from_dir(model_dir: Path, device: torch.device, is_trainable: bool = False) -> nn.Module:
    base_model = AutoModelForSequenceClassification.from_pretrained(MODEL_NAME, num_labels=2)
    model = PeftModel.from_pretrained(base_model, model_dir, is_trainable=is_trainable)
    model.to(device)
    return model


def load_best_lora_model(device: torch.device) -> nn.Module:
    return load_lora_model_from_dir(BEST_MODEL_DIR, device, is_trainable=False)
...
def main() -> None:
    ...
    num_update_steps_per_epoch = math.ceil(len(train_dataset) / train_batch_size / grad_accum_steps)
    ...
    best_model = load_best_lora_model(device)
