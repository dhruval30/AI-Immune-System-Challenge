#!/usr/bin/env python3
"""
Train RoBERTa with hard-example sample weighting after label-noise cleaning.
...
The script keeps the proven roberta-base setup and normal CE objective, but uses
per-row sample weights so the model pays more attention to the hard regimes.

Before splitting/training, it removes high-confidence contradictory rows using
existing out-of-fold RoBERTa CV probabilities:
- TRUE with OOF prob_TRUE <= 0.05
- FALSE with OOF prob_TRUE >= 0.95
"""
...
OUTPUT_DIR = ROOT_DIR / "outputs" / "roberta_hard_weighted_cleaned"
BEST_MODEL_DIR = OUTPUT_DIR / "best_model"
BEST_STATE_DICT_PATH = OUTPUT_DIR / "roberta_hard_weighted_cleaned_best_state_dict.pt"
...
TRAIN_PROB_PATH = ROOT_DIR / "outputs" / "roberta_base_cv_v2" / "roberta_base_cv_v2_oof_predictions.csv"
TRAIN_PROB_COLUMN = "oof_prob_TRUE_mean"
TRUE_NOISE_PROB_MAX = 0.05
FALSE_NOISE_PROB_MIN = 0.95

METRICS_PATH = OUTPUT_DIR / "roberta_hard_weighted_cleaned_metrics.json"
VAL_PRED_PATH = OUTPUT_DIR / "roberta_hard_weighted_cleaned_val_predictions.csv"
ERROR_ANALYSIS_PATH = OUTPUT_DIR / "roberta_hard_weighted_cleaned_error_analysis.csv"
TEST_PROB_PATH = OUTPUT_DIR / "roberta_hard_weighted_cleaned_test_probabilities.csv"
SUBMISSION_PATH = OUTPUT_DIR / "roberta_hard_weighted_cleaned_submission.csv"
NOTES_PATH = OUTPUT_DIR / "roberta_hard_weighted_cleaned_notes.md"
WEIGHT_SUMMARY_PATH = OUTPUT_DIR / "roberta_hard_weighted_cleaned_weight_summary.csv"
FEATURE_THRESHOLDS_PATH = OUTPUT_DIR / "roberta_hard_weighted_cleaned_feature_thresholds.json"
CLEANING_SUMMARY_PATH = OUTPUT_DIR / "roberta_hard_weighted_cleaned_cleaning_summary.json"
CLEANED_ROWS_PATH = OUTPUT_DIR / "roberta_hard_weighted_cleaned_removed_rows.csv"
...
def load_train_probabilities(expected_rows: int) -> pd.DataFrame:
    if not TRAIN_PROB_PATH.exists():
        raise FileNotFoundError(
            f"Training probability file not found: {TRAIN_PROB_PATH}. "
            "Expected out-of-fold train probabilities so label-noise filtering does not use in-fold predictions."
        )

    prob_df = pd.read_csv(TRAIN_PROB_PATH)
    if len(prob_df) != expected_rows:
        raise ValueError(
            f"Training probability rows ({len(prob_df)}) do not match train rows ({expected_rows}): {TRAIN_PROB_PATH}"
        )
    if TRAIN_PROB_COLUMN not in prob_df.columns:
        raise ValueError(
            f"Training probability file must contain column {TRAIN_PROB_COLUMN!r}; found {list(prob_df.columns)}"
        )
    if "text" not in prob_df.columns:
        raise ValueError(f"Training probability file must contain text column for alignment checks: {TRAIN_PROB_PATH}")

    prob_df = prob_df.copy()
    prob_df["text"] = [normalize_text(value) for value in prob_df["text"]]
    prob_df[TRAIN_PROB_COLUMN] = pd.to_numeric(prob_df[TRAIN_PROB_COLUMN], errors="coerce")
    if prob_df[TRAIN_PROB_COLUMN].isna().any():
        bad_rows = int(prob_df[TRAIN_PROB_COLUMN].isna().sum())
        raise ValueError(f"Found {bad_rows} missing/non-numeric probabilities in {TRAIN_PROB_COLUMN}.")
    return prob_df


def clean_training_rows(train_df: pd.DataFrame, y: np.ndarray) -> tuple[pd.DataFrame, np.ndarray, dict]:
    prob_df = load_train_probabilities(expected_rows=len(train_df))

    train_texts_normalized = [normalize_text(value) for value in train_df["text"]]
    text_matches = np.array(train_texts_normalized) == prob_df["text"].to_numpy(dtype=str)
    text_match_rate = float(text_matches.mean())
    if text_match_rate < 0.995:
        mismatch_count = int((~text_matches).sum())
        raise ValueError(
            f"Train probability file does not align by row order. Text match rate={text_match_rate:.6f}, "
            f"mismatches={mismatch_count}. Refusing to clean labels from misaligned probabilities."
        )

    prob_true = prob_df[TRAIN_PROB_COLUMN].to_numpy(dtype=float)
    true_noise_mask = (y == 1) & (prob_true <= TRUE_NOISE_PROB_MAX)
    false_noise_mask = (y == 0) & (prob_true >= FALSE_NOISE_PROB_MIN)
    remove_mask = true_noise_mask | false_noise_mask
    keep_mask = ~remove_mask

    removed_df = train_df.loc[remove_mask].copy()
    removed_df["encoded_label"] = y[remove_mask]
    removed_df["label_normalized"] = decode_labels(y[remove_mask])
    removed_df["oof_prob_TRUE"] = prob_true[remove_mask]
    removed_df["removal_reason"] = np.where(
        true_noise_mask[remove_mask],
        f"TRUE_prob_TRUE_le_{TRUE_NOISE_PROB_MAX}",
        f"FALSE_prob_TRUE_ge_{FALSE_NOISE_PROB_MIN}",
    )
    removed_df.to_csv(CLEANED_ROWS_PATH, index=False)

    cleaned_df = train_df.loc[keep_mask].reset_index(drop=True)
    cleaned_y = y[keep_mask]
    original_distribution = {
        "FALSE": int((y == 0).sum()),
        "TRUE": int((y == 1).sum()),
    }
    cleaned_distribution = {
        "FALSE": int((cleaned_y == 0).sum()),
        "TRUE": int((cleaned_y == 1).sum()),
    }
    removed_distribution = {
        "FALSE": int(((y == 0) & remove_mask).sum()),
        "TRUE": int(((y == 1) & remove_mask).sum()),
    }
    summary = {
        "prediction_source": str(TRAIN_PROB_PATH),
        "prediction_column": TRAIN_PROB_COLUMN,
        "alignment": {
            "method": "row_order_with_text_validation",
            "text_match_rate": text_match_rate,
            "mismatched_rows": int((~text_matches).sum()),
        },
        "thresholds": {
            "remove_TRUE_if_prob_TRUE_lte": TRUE_NOISE_PROB_MAX,
            "remove_FALSE_if_prob_TRUE_gte": FALSE_NOISE_PROB_MIN,
        },
        ...
        "removed_label_distribution": removed_distribution,
        "final_label_distribution": cleaned_distribution,
        "removed_rows_path": str(CLEANED_ROWS_PATH),
    }
    CLEANING_SUMMARY_PATH.write_text(json.dumps(summary, indent=2), encoding="utf-8")
    return cleaned_df, cleaned_y, summary
...
def main() -> None:
    ...
    print(INSTALL_CMD)
    print("\n=== RoBERTa Hard-Weighted Training On Cleaned Data ===")
    ...
    print("\n=== Preparing text, labels, cleaning, and hard weights ===")
    test_texts = normalize_texts(test_df["text"], desc="Normalizing test text")
    y = encode_labels(train_df["label"], desc="Encoding labels")
    print("\n=== Removing high-confidence likely label noise ===")
    train_df, y, cleaning_summary = clean_training_rows(train_df, y)
    print(f"Removed rows: {cleaning_summary['removed_rows']}")
    print(f"Final training rows before split: {cleaning_summary['final_rows']}")
    print(f"Final label distribution: {cleaning_summary['final_label_distribution']}")
    print(f"Saved cleaning summary: {CLEANING_SUMMARY_PATH}")
    print(f"Saved removed rows: {CLEANED_ROWS_PATH}")

    texts = normalize_texts(train_df["text"], desc="Normalizing cleaned train text")
    ...
    metrics_payload = {
        ...
        "paths": {
            ...
            "removed_rows": str(CLEANED_ROWS_PATH),
    ...
    notes_text = f"""# RoBERTa Hard-Weighted Cleaned-Data Notes
...
- Prediction source: `{TRAIN_PROB_PATH.relative_to(ROOT_DIR)}`
- Probability column: `{TRAIN_PROB_COLUMN}`
- Remove TRUE if `prob_TRUE <= {TRUE_NOISE_PROB_MAX}`
- Remove FALSE if `prob_TRUE >= {FALSE_NOISE_PROB_MIN}`
...
- Original label distribution: `{cleaning_summary['original_label_distribution']}`
- Removed label distribution: `{cleaning_summary['removed_label_distribution']}`
- Final label distribution: `{cleaning_summary['final_label_distribution']}`
