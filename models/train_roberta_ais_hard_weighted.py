#!/usr/bin/env python3
"""
Train RoBERTa with AIS-aware hard-example sample weighting.

Dependency install command:
pip install pandas numpy scikit-learn tqdm torch transformers accelerate

This is a targeted experiment after reviewing the AIS reference material.
The intended target is concealed deviant AI-agent behavior: collusion, cryptic
coordination, manipulation, or unsafe coordination inside apparently ordinary
agent conversations.

The script keeps the proven roberta-base setup and normal CE objective, but uses
conservative per-row sample weights so the model pays more attention to hard
semantic regimes without becoming a pure weirdness detector.
"""
...
OUTPUT_DIR = ROOT_DIR / "outputs" / "roberta_ais_hard_weighted"
BEST_MODEL_DIR = OUTPUT_DIR / "best_model"
BEST_STATE_DICT_PATH = OUTPUT_DIR / "roberta_ais_hard_weighted_best_state_dict.pt"
...
METRICS_PATH = OUTPUT_DIR / "roberta_ais_hard_weighted_metrics.json"
VAL_PRED_PATH = OUTPUT_DIR / "roberta_ais_hard_weighted_val_predictions.csv"
ERROR_ANALYSIS_PATH = OUTPUT_DIR / "roberta_ais_hard_weighted_error_analysis.csv"
TEST_PROB_PATH = OUTPUT_DIR / "roberta_ais_hard_weighted_test_probabilities.csv"
SUBMISSION_PATH = OUTPUT_DIR / "roberta_ais_hard_weighted_submission.csv"
NOTES_PATH = OUTPUT_DIR / "roberta_ais_hard_weighted_notes.md"
WEIGHT_SUMMARY_PATH = OUTPUT_DIR / "roberta_ais_hard_weighted_weight_summary.csv"
FEATURE_THRESHOLDS_PATH = OUTPUT_DIR / "roberta_ais_hard_weighted_feature_thresholds.json"
AIS_FEATURE_COLUMNS = [
...
INSURANCE_RE = re.compile(
    r"\b("
    r"insurance|insurer|insured|policy|premium|claim|claims|coverage|liability|"
    r"deductible|adjuster|underwriting|settlement|payout|compensation|assessment|"
    r"valuation|appraisal|damage|damages"
    r")\b",
    re.IGNORECASE,
)
COORDINATION_RE = re.compile(
    r"\b("
    r"coordinate|coordination|cooperate|cooperation|collaborate|collaboration|"
    r"agree|agreement|arrange|align|joint|together|mutual|share|split|signal|"
    r"timing|schedule|sequence|turn|round|between us"
    r")\b",
    re.IGNORECASE,
)
MANIPULATION_RE = re.compile(
    r"\b("
    r"collude|collusion|cartel|inflate|deflate|fix|rig|manipulate|manipulation|"
    r"deceive|deception|mislead|fraud|fake|conceal|hide|secret|kickback|bribe|"
    r"exclude|competitor|monopoly|monopolize|overstate|understate"
    r")\b",
    re.IGNORECASE,
)
CRYPTIC_RE = re.compile(
    r"\b("
    r"code word|coded|cryptic|hidden|subtle|quiet|silently|do not mention|"
    r"don't mention|off record|cover story|mask|disguise|innocuous|indirect|"
    r"without saying|between the lines"
    r")\b",
    re.IGNORECASE,
)
AGENT_RE = re.compile(
    r"\b(user|assistant|system|agent|agents|tool|tools|message|messages|instruction|prompt)\b|"
    r"\[[0-9]+_(?:system|user|assistant)",
    re.IGNORECASE,
)
...
def count_regex_matches(pattern: re.Pattern, text: str) -> int:
...
def extract_features(text: str) -> dict[str, float]:
    ...
    insurance_term_count = count_regex_matches(INSURANCE_RE, text)
    coordination_term_count = count_regex_matches(COORDINATION_RE, text)
    manipulation_term_count = count_regex_matches(MANIPULATION_RE, text)
    cryptic_term_count = count_regex_matches(CRYPTIC_RE, text)
    agent_marker_count = count_regex_matches(AGENT_RE, text)
    ...
    return {
        ...
        "insurance_term_count": float(insurance_term_count),
        "coordination_term_count": float(coordination_term_count),
        "manipulation_term_count": float(manipulation_term_count),
        "cryptic_term_count": float(cryptic_term_count),
        "agent_marker_count": float(agent_marker_count),
    }
...
def ais_semantic_score(row: pd.Series) -> int:
    """Lightweight AIS/collusion cue score. This is intentionally conservative."""
    score = 0
    if row["manipulation_term_count"] > 0:
        score += 2
    if row["coordination_term_count"] > 0:
        score += 1
    if row["insurance_term_count"] > 0:
        score += 1
    if row["cryptic_term_count"] > 0:
        score += 1
    if row["agent_marker_count"] > 0 and (row["coordination_term_count"] > 0 or row["cryptic_term_count"] > 0):
        score += 1
    return int(min(score, 6))


def assign_sample_weight(label: int, abnormality: int, ais_score: int) -> tuple[float, str]:
    """Conservative hard-example weighting.

    The best baseline used normal CE. The old hard-weighted script helped locally but was
    mostly artifact/noise driven. This variant nudges hard AIS regimes without turning
    the loss into aggressive class weighting.
    """
    if label == 1:
        if ais_score >= 2 and abnormality <= 2:
            return 1.9, "hard_ais_clean_TRUE"
        if abnormality <= 1:
            return 1.7, "hard_clean_TRUE"
        if ais_score >= 2:
            return 1.4, "ais_signal_TRUE"
        if abnormality >= 4 and ais_score == 0:
            return 0.95, "easy_artifact_noisy_TRUE"
        return 1.05, "standard_TRUE"

    if ais_score >= 2 and abnormality <= 2:
        return 1.6, "hard_benign_coordination_FALSE"
    if abnormality >= 3:
        return 1.45, "hard_messy_FALSE"
    if abnormality == 2:
        return 1.2, "medium_messy_FALSE"
    if abnormality == 0 and ais_score == 0:
        return 0.9, "easy_clean_FALSE"
...
def build_weight_frame(
    ...
    for label, (_, feature_row) in zip(labels, feature_df.iterrows(), strict=True):
        surface_score = abnormal_score(feature_row, thresholds)
        semantic_score = ais_semantic_score(feature_row)
        raw_weight, category = assign_sample_weight(int(label), surface_score, semantic_score)
        rows.append(
            {
                "abnormal_score": surface_score,
                "ais_semantic_score": semantic_score,
...
def write_weight_summary(
    ...
    grouped = (
        ...
        .agg(
            ...
            mean_ais_semantic_score=("ais_semantic_score", "mean"),
...
def main() -> None:
    ...
    print(INSTALL_CMD)
    print("\n=== RoBERTa AIS Hard-Weighted Training ===")
    ...
    print("Weighting logic: conservative AIS semantic hard-example weighting plus surface abnormality weighting.")
    ...
    test_ais_scores = [ais_semantic_score(row) for _, row in test_features.iterrows()]
    ...
    val_pred_df = pd.DataFrame(
        {
            ...
            "ais_semantic_score": val_weight_df["ais_semantic_score"].to_numpy(),
            "hard_category": val_weight_df["hard_category"].to_numpy(),
            "raw_sample_weight_if_trained": val_weight_df["raw_sample_weight"].to_numpy(),
        }
    )
    for column in AIS_FEATURE_COLUMNS:
        val_pred_df[column] = val_features[column].to_numpy()
    ...
    test_prob_df = pd.DataFrame(
        {
            ...
            "ais_semantic_score": test_ais_scores,
        }
    )
    for column in AIS_FEATURE_COLUMNS:
        test_prob_df[column] = test_features[column].to_numpy()
    ...
    metrics_payload = {
        ...
        "config": {
            ...
            "weight_strategy": {
                "hard_ais_clean_TRUE": 1.9,
                "hard_clean_TRUE": 1.7,
                "ais_signal_TRUE": 1.4,
                "standard_TRUE": 1.05,
                "easy_artifact_noisy_TRUE": 0.95,
                "hard_benign_coordination_FALSE": 1.6,
                "hard_messy_FALSE": 1.45,
                "medium_messy_FALSE": 1.2,
                "easy_clean_FALSE": 0.9,
                "normalization": "divide train split raw weights by train split mean weight",
            },
            "feature_thresholds": thresholds,
            "ais_semantic_features": AIS_FEATURE_COLUMNS,
        },
        "split": {
    ...
    notes_text = f"""# RoBERTa AIS Hard-Weighted Notes
