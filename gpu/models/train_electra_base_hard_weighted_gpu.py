#!/usr/bin/env python3
"""
Train ELECTRA-base with hard-example sample weighting.

Dependency install command:
pip install pandas numpy scikit-learn tqdm torch transformers accelerate

This is a direct backbone swap of the proven hard-weighted GPU RoBERTa run.

The hard-example weighting recipe, train/validation split, max length, loss,
learning rate, and training loop are intentionally kept the same. Only the
encoder is changed to ELECTRA (`google/electra-base-discriminator` or a local
copy at `/workspace/gpu/electra-base-discriminator`).
"""
...
SEED = 42
LOCAL_MODEL_NAME = "/workspace/gpu/electra-base-discriminator"
HF_MODEL_NAME = "google/electra-base-discriminator"
...
OUTPUT_DIR = ROOT_DIR / "outputs" / "electra_base_hard_weighted_gpu"
BEST_MODEL_DIR = OUTPUT_DIR / "best_model"
BEST_STATE_DICT_PATH = OUTPUT_DIR / "electra_base_hard_weighted_gpu_best_state_dict.pt"
...
METRICS_PATH = OUTPUT_DIR / "electra_base_hard_weighted_gpu_metrics.json"
VAL_PRED_PATH = OUTPUT_DIR / "electra_base_hard_weighted_gpu_val_predictions.csv"
ERROR_ANALYSIS_PATH = OUTPUT_DIR / "electra_base_hard_weighted_gpu_error_analysis.csv"
TEST_PROB_PATH = OUTPUT_DIR / "electra_base_hard_weighted_gpu_test_probabilities.csv"
SUBMISSION_PATH = OUTPUT_DIR / "electra_base_hard_weighted_gpu_submission.csv"
SUBMISSION_THR032_PATH = OUTPUT_DIR / "electra_base_hard_weighted_gpu_submission_thr0p32.csv"
NOTES_PATH = OUTPUT_DIR / "electra_base_hard_weighted_gpu_notes.md"
WEIGHT_SUMMARY_PATH = OUTPUT_DIR / "electra_base_hard_weighted_gpu_weight_summary.csv"
FEATURE_THRESHOLDS_PATH = OUTPUT_DIR / "electra_base_hard_weighted_gpu_feature_thresholds.json"
...
def main() -> None:
    ...
    print(INSTALL_CMD)
    print("\n=== ELECTRA-base Hard-Weighted Training ===")
    ...
    print(f"Local ELECTRA path exists: {Path(LOCAL_MODEL_NAME).exists()}")
    ...
    fixed_pred = (test_prob_true >= fixed_threshold).astype(int)
    fixed_pred_labels = decode_labels(fixed_pred)
    fixed_distribution = {
        "pred_FALSE": int((fixed_pred == 0).sum()),
        "pred_TRUE": int((fixed_pred == 1).sum()),
    }
    if solution_df.columns.tolist() == ["label"]:
        fixed_submission_df = pd.DataFrame({"label": fixed_pred_labels})
    else:
        fixed_submission_df = solution_df.copy()
        fixed_submission_df["label"] = fixed_pred_labels
    ...
    print(f"Saved fixed threshold 0.32 submission: {SUBMISSION_THR032_PATH}")
    ...
    metrics_payload = {
        ...
        "config": {
            ...
            "adam_eps": ADAM_EPS,
            "fixed_submission_threshold": fixed_threshold,
            "optimizer": "AdamW",
    ...
    notes_text = f"""# ELECTRA-base Hard-Weighted Notes

## Why This Run Exists

The current best run is the GPU hard-weighted RoBERTa model, public LB `0.9347079` at threshold `0.32`.
DeBERTa was unstable/low-calibrated in this environment, so this script tests ELECTRA as the next clean
backbone swap while preserving the hard-weighted training recipe.

This is not a brute-force sweep. It keeps the hard-example weights, split, max length, optimizer setup,
and training loop from `gpu/models/train_roberta_hard_weighted_gpu.py`, but changes the encoder to
ELECTRA-base.
...
- Loss: `sample_weighted_cross_entropy`
- Local model path preferred: `{LOCAL_MODEL_NAME}`
- HF fallback model: `{HF_MODEL_NAME}`
- Fixed threshold submission: `0.32`
- Hard weights:
  - clean TRUE: `2.5`
