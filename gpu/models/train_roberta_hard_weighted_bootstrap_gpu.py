#!/usr/bin/env python3
"""
Train RoBERTa with hard-example sample weighting and bootstrapped CE.

Dependency install command:
pip install pandas numpy scikit-learn tqdm torch transformers accelerate

This is a label-noise-robust variant of the current best hard-weighted GPU
script. It keeps the same model, data split, hard-example weights, and inference
flow, but changes the training objective:

- epoch 1: normal hard-weighted cross entropy
- epoch 2+: hard-weighted bootstrapped cross entropy

Bootstrapped CE mixes the ground-truth one-hot label with the model's detached
probability distribution. This reduces the penalty from potentially mislabeled
or contradictory rows without deleting or flipping labels.
"""
...
OUTPUT_DIR = ROOT_DIR / "outputs" / "roberta_hard_weighted_bootstrap_gpu"
BEST_MODEL_DIR = OUTPUT_DIR / "best_model"
BEST_STATE_DICT_PATH = OUTPUT_DIR / "roberta_hard_weighted_bootstrap_gpu_best_state_dict.pt"
...
METRICS_PATH = OUTPUT_DIR / "roberta_hard_weighted_bootstrap_gpu_metrics.json"
VAL_PRED_PATH = OUTPUT_DIR / "roberta_hard_weighted_bootstrap_gpu_val_predictions.csv"
ERROR_ANALYSIS_PATH = OUTPUT_DIR / "roberta_hard_weighted_bootstrap_gpu_error_analysis.csv"
TEST_PROB_PATH = OUTPUT_DIR / "roberta_hard_weighted_bootstrap_gpu_test_probabilities.csv"
SUBMISSION_PATH = OUTPUT_DIR / "roberta_hard_weighted_bootstrap_gpu_submission.csv"
FIXED_SUBMISSION_PATH = OUTPUT_DIR / "roberta_hard_weighted_bootstrap_gpu_submission_thr0p32.csv"
NOTES_PATH = OUTPUT_DIR / "roberta_hard_weighted_bootstrap_gpu_notes.md"
WEIGHT_SUMMARY_PATH = OUTPUT_DIR / "roberta_hard_weighted_bootstrap_gpu_weight_summary.csv"
FEATURE_THRESHOLDS_PATH = OUTPUT_DIR / "roberta_hard_weighted_bootstrap_gpu_feature_thresholds.json"
...
def bootstrapped_cross_entropy(
    ...
    """Per-row bootstrapped CE for noisy labels.

    beta=1.0 is normal CE. Lower beta trusts the model distribution more. The
    model probabilities are detached so the soft target does not create a
    self-reinforcing gradient path through the target itself.
    """
    num_classes = logits.size(-1)
    hard_targets = F.one_hot(labels, num_classes=num_classes).to(dtype=logits.dtype)
    soft_targets = torch.softmax(logits.detach(), dim=-1)
    mixed_targets = beta * hard_targets + (1.0 - beta) * soft_targets
    log_probs = F.log_softmax(logits, dim=-1)
    return -(mixed_targets * log_probs).sum(dim=-1)
...
def main() -> None:
    ...
    print(INSTALL_CMD)
    print("\n=== RoBERTa Hard-Weighted Bootstrapped CE Training ===")
    ...
    print("Loss: hard-weighted CE for epoch 1, then hard-weighted bootstrapped CE")
    print(f"Bootstrap start epoch: {BOOTSTRAP_START_EPOCH}")
    print(f"Bootstrap beta: {BOOTSTRAP_BETA}")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        using_bootstrap = epoch >= BOOTSTRAP_START_EPOCH
        for step, batch in enumerate(progress, start=1):
            ...
            outputs = model(input_ids=input_ids, attention_mask=attention_mask)
            raw_ce_loss = F.cross_entropy(outputs.logits, labels, reduction="none")
            if using_bootstrap:
                per_row_loss = bootstrapped_cross_entropy(outputs.logits, labels, beta=BOOTSTRAP_BETA)
            else:
                per_row_loss = raw_ce_loss
            ...
            running_unweighted_loss += float(raw_ce_loss.mean().detach().cpu().item()) * batch_size
            ...
            progress.set_postfix(
                ...
                objective="boot_ce" if using_bootstrap else "ce",
        ...
        val_metrics_05["training_objective"] = "bootstrapped_cross_entropy" if using_bootstrap else "cross_entropy"
        ...
        print(
            ...
            f"| objective={val_metrics_05['training_objective']} "
            ...
            torch.save(
                {
                    ...
                    "bootstrap_start_epoch": BOOTSTRAP_START_EPOCH,
    ...
    metrics_payload = {
        ...
        "config": {
            ...
            "loss": "sample_weighted_cross_entropy_then_bootstrapped_cross_entropy",
            "bootstrap_start_epoch": BOOTSTRAP_START_EPOCH,
            "bootstrap_beta": BOOTSTRAP_BETA,
            "weight_strategy": {
                "hard_clean_TRUE": 2.5,
        ...
        "comparison_target": {
            "submission": "gpu/outputs/roberta_hard_weighted_gpu_b64_final/roberta_hard_weighted_gpu_b64_submission_thr0p32.csv",
            "public_lb": 0.9347079,
            "threshold": 0.32,
        },
    }
    ...
    notes_text = f"""# RoBERTa Hard-Weighted Bootstrapped CE Notes
...
- Loss epoch {BOOTSTRAP_START_EPOCH}+: `sample_weighted_bootstrapped_cross_entropy`
- Bootstrap beta: `{BOOTSTRAP_BETA}`
...
This run should be judged first by its fixed-threshold `{FIXED_SUBMISSION_THRESHOLD:.2f}` submission.
