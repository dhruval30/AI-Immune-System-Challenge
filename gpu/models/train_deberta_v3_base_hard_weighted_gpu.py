#!/usr/bin/env python3
"""
Train DeBERTa-v3-base with hard-example sample weighting.

Dependency install command:
pip install pandas numpy scikit-learn tqdm torch transformers accelerate sentencepiece protobuf
...
This is a DeBERTa-specific version of the proven hard-weighted GPU RoBERTa run.
The hard-example weighting recipe, split, loss, max length, and threshold search
are kept, but CUDA math and optimizer/batch settings are made more conservative
because DeBERTa-v3-base produced NaNs with the direct RoBERTa hyperparameters.
"""
...
from transformers import AutoModelForSequenceClassification, DebertaV2Tokenizer, get_linear_schedule_with_warmup


INSTALL_CMD = "pip install pandas numpy scikit-learn tqdm torch transformers accelerate sentencepiece protobuf"

os.environ["TOKENIZERS_PARALLELISM"] = "false"

SEED = 42
MODEL_NAME = "/workspace/gpu/deberta-v3-base"
MAX_LENGTH = 384
EPOCHS = 6
TRAIN_BATCH_SIZE = 4
EVAL_BATCH_SIZE = 16
...
ADAM_EPS = 1e-6
MAX_GRAD_NORM = 0.5
SUBMISSION_THRESHOLD = 0.32

THRESHOLD_GRID = np.round(np.arange(0.05, 0.701, 0.01), 2)

ROOT_DIR = Path(__file__).resolve().parents[1]
DATA_DIR = ROOT_DIR / "data"
OUTPUT_DIR = ROOT_DIR / "outputs" / "deberta_v3_base_hard_weighted_gpu"
BEST_MODEL_DIR = OUTPUT_DIR / "best_model"
BEST_STATE_DICT_PATH = OUTPUT_DIR / "deberta_v3_base_hard_weighted_gpu_best_state_dict.pt"
...
METRICS_PATH = OUTPUT_DIR / "deberta_v3_base_hard_weighted_gpu_metrics.json"
VAL_PRED_PATH = OUTPUT_DIR / "deberta_v3_base_hard_weighted_gpu_val_predictions.csv"
ERROR_ANALYSIS_PATH = OUTPUT_DIR / "deberta_v3_base_hard_weighted_gpu_error_analysis.csv"
TEST_PROB_PATH = OUTPUT_DIR / "deberta_v3_base_hard_weighted_gpu_test_probabilities.csv"
SUBMISSION_PATH = OUTPUT_DIR / "deberta_v3_base_hard_weighted_gpu_submission.csv"
SUBMISSION_THR032_PATH = OUTPUT_DIR / "deberta_v3_base_hard_weighted_gpu_submission_thr0p32.csv"
NOTES_PATH = OUTPUT_DIR / "deberta_v3_base_hard_weighted_gpu_notes.md"
WEIGHT_SUMMARY_PATH = OUTPUT_DIR / "deberta_v3_base_hard_weighted_gpu_weight_summary.csv"
FEATURE_THRESHOLDS_PATH = OUTPUT_DIR / "deberta_v3_base_hard_weighted_gpu_feature_thresholds.json"
...
def configure_cuda_runtime() -> None:
    if not torch.cuda.is_available():
        return
    # DeBERTa-v3 has been less numerically forgiving than RoBERTa in this env.
    # Use conservative fp32 CUDA math instead of TF32 / fused SDP shortcuts.
...
def validate_local_deberta_files(model_name: str) -> None:
    ...
    if missing_files:
        raise FileNotFoundError(
            f"Local DeBERTa directory is missing files: {missing_files}. "
            f"Expected them under: {model_path}"
        )


def load_deberta_tokenizer(model_name: str) -> DebertaV2Tokenizer:
    try:
        tokenizer = DebertaV2Tokenizer.from_pretrained(model_name, local_files_only=Path(model_name).exists())
        if tokenizer.pad_token is None:
            tokenizer.pad_token = tokenizer.eos_token
        return tokenizer
    except ImportError as exc:
        raise ImportError(
            "DeBERTa-v3 uses a SentencePiece tokenizer. Install it in the GPU environment with: "
...
def compute_metrics(y_true: np.ndarray, prob_true: np.ndarray, threshold: float) -> dict:
    if not np.isfinite(prob_true).all():
        non_finite_count = int((~np.isfinite(prob_true)).sum())
        raise ValueError(f"Cannot compute metrics: validation probabilities contain {non_finite_count} non-finite values.")
...
def validate_model_parameters(model: AutoModelForSequenceClassification) -> None:
    bad_tensors = []
    largest_abs = 0.0
    largest_name = ""
    for name, param in model.named_parameters():
        ...
        if not torch.isfinite(data).all():
            ...
            continue
        if data.numel() > 0:
            max_abs = float(data.abs().max().cpu().item())
            if max_abs > largest_abs:
                largest_abs = max_abs
                largest_name = name

    if bad_tensors:
        preview = bad_tensors[:10]
        raise RuntimeError(
            f"Loaded DeBERTa checkpoint contains non-finite weights in {len(bad_tensors)} tensors. "
            f"First bad tensors: {preview}. Re-download /workspace/gpu/deberta-v3-base/pytorch_model.bin."
        )
    if largest_abs > 1_000:
        raise RuntimeError(
            f"Loaded DeBERTa checkpoint has suspiciously large weights: {largest_name} max_abs={largest_abs:.2f}. "
            "This usually means the local pytorch_model.bin is corrupted or incompatible."
        )
    print(f"Model parameter finite check passed. Largest |weight|: {largest_abs:.4f} ({largest_name})")


def run_forward_finite_check(
    model: AutoModelForSequenceClassification,
    dataset: TensorDataset,
    device: torch.device,
) -> None:
    model.eval()
    loader = DataLoader(dataset, batch_size=min(EVAL_BATCH_SIZE, len(dataset)), shuffle=False)
    with torch.no_grad():
        ...
        if not torch.isfinite(logits).all():
            non_finite = int((~torch.isfinite(logits)).sum().detach().cpu().item())
            raise RuntimeError(
                f"DeBERTa produced {non_finite} non-finite logits before training. "
                "This is not a training hyperparameter issue. Check the downloaded checkpoint files "
                "or try a different torch/transformers build."
            )
        loss = F.cross_entropy(logits, labels)
        if not torch.isfinite(loss):
            raise RuntimeError(
                "DeBERTa produced non-finite loss before training. "
                "Check the local checkpoint/tokenizer compatibility."
            )
    model.train()
    print(f"Initial forward finite check passed. Initial CE loss: {float(loss.detach().cpu().item()):.6f}")
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc=desc, unit="batch"):
            ...
            if not torch.isfinite(outputs.logits).all():
                raise RuntimeError(f"Non-finite logits encountered during {desc}.")
            probs = torch.softmax(outputs.logits, dim=1)[:, 1]
            if not torch.isfinite(probs).all():
                raise RuntimeError(f"Non-finite probabilities encountered during {desc}.")
            all_probs.append(probs.detach().cpu().numpy())
            if has_labels and labels is not None:
                losses = F.cross_entropy(outputs.logits, labels, reduction="none")
                if not torch.isfinite(losses).all():
                    raise RuntimeError(f"Non-finite evaluation loss encountered during {desc}.")
...
def main() -> None:
    ...
    print(INSTALL_CMD)
    print("\n=== DeBERTa-v3-base Hard-Weighted Training ===")
    ...
    print(f"Local model path exists: {Path(MODEL_NAME).exists()}")
    print("Loss: per-row CrossEntropyLoss weighted by hard-example category")
    print("DeBERTa-safe settings: TF32 disabled, flash/mem-efficient SDP disabled where available")
    validate_local_deberta_files(MODEL_NAME)
    ...
    tokenizer = load_deberta_tokenizer(MODEL_NAME)
    ...
    run_forward_finite_check(model=model, dataset=val_dataset, device=device)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for step, batch in enumerate(progress, start=1):
            ...
            if not torch.isfinite(outputs.logits).all():
                ...
                progress.set_postfix(
                    train_loss="non_finite_logits",
                    skipped_loss=non_finite_loss_batches,
                    skipped_grad=non_finite_grad_steps,
                    lr=f"{scheduler.get_last_lr()[0]:.2e}",
                )
                if non_finite_loss_batches >= 5 and seen_examples == 0:
                    raise RuntimeError(
                        "DeBERTa produced non-finite logits in the first training batches. "
                        "This run is numerically broken before learning starts."
                    )
                continue

            per_row_loss = F.cross_entropy(outputs.logits, labels, reduction="none")
            if not torch.isfinite(per_row_loss).all():
                non_finite_loss_batches += 1
                optimizer.zero_grad(set_to_none=True)
                progress.set_postfix(
                    train_loss="non_finite_row_loss",
                    skipped_loss=non_finite_loss_batches,
                    skipped_grad=non_finite_grad_steps,
                    lr=f"{scheduler.get_last_lr()[0]:.2e}",
                )
                if non_finite_loss_batches >= 5 and seen_examples == 0:
                    raise RuntimeError(
                        "DeBERTa produced non-finite row losses in the first training batches. "
                        "This usually indicates a checkpoint/runtime numerical incompatibility."
                    )
                continue
            ...
            if not math.isfinite(loss_value):
                ...
                if non_finite_loss_batches >= 5 and seen_examples == 0:
                    raise RuntimeError(
                        "DeBERTa produced non-finite weighted loss in the first training batches. "
                        "Lowering LR will not help if this happens before any optimizer step."
                    )
                continue
        ...
        epoch_tuned_metrics = compute_metrics(
            y_true=val_true,
            prob_true=val_prob_true,
            threshold=epoch_best_threshold,
        )
        val_metrics_05["best_tuned_threshold"] = float(epoch_best_threshold)
        val_metrics_05["best_tuned_f1"] = float(epoch_tuned_metrics["f1"])
        val_metrics_05["best_tuned_precision"] = float(epoch_tuned_metrics["precision"])
        val_metrics_05["best_tuned_recall"] = float(epoch_tuned_metrics["recall"])
        val_metrics_05["best_tuned_prediction_distribution"] = epoch_tuned_metrics["prediction_distribution"]
        ...
        if epoch_tuned_metrics["f1"] > best_epoch_f1:
            ...
            torch.save(
                {
                    "epoch": best_epoch,
                    "best_tuned_val_f1": best_epoch_f1,
                    "best_tuned_threshold": best_epoch_threshold,
            ...
            print(
                ...
                f"with best_tuned_f1={best_epoch_f1:.6f} at threshold={best_epoch_threshold:.2f}"
            )
    ...
    fixed_pred = (test_prob_true >= SUBMISSION_THRESHOLD).astype(int)
    fixed_labels = decode_labels(fixed_pred)
    if solution_df.columns.tolist() == ["label"]:
        fixed_submission_df = pd.DataFrame({"label": fixed_labels})
    else:
        fixed_submission_df = solution_df.copy()
        fixed_submission_df["label"] = fixed_labels
    ...
    fixed_distribution = {
        "pred_FALSE": int((fixed_pred == 0).sum()),
        "pred_TRUE": int((fixed_pred == 1).sum()),
    }
    print(f"Saved fixed threshold {SUBMISSION_THRESHOLD:.2f} submission: {SUBMISSION_THR032_PATH}")
    ...
    metrics_payload = {
        ...
        "best_epoch": {
            ...
            "best_tuned_val_f1": float(best_epoch_f1),
            "best_tuned_threshold": float(best_epoch_threshold),
        ...
        "comparison_target": {
            "baseline_submission": "outputs/roberta_base/roberta_base_submission.csv",
            "baseline_public_lb": 0.90909091,
            "best_hard_weighted_gpu_submission": "gpu/outputs/roberta_hard_weighted_gpu_b64_final/roberta_hard_weighted_gpu_b64_submission_thr0p32.csv",
            "best_hard_weighted_gpu_public_lb": 0.9347079,
        },
    }
    METRICS_PATH.write_text(json.dumps(metrics_payload, indent=2), encoding="utf-8")
    print(f"Saved metrics: {METRICS_PATH}")

    notes_text = f"""# DeBERTa-v3-base Hard-Weighted Notes

## Why This Run Exists

The current best run is hard-weighted RoBERTa on GPU with public LB `0.9347079` at threshold `0.32`.
This script started as a direct architecture swap, but DeBERTa-v3-base produced NaNs with the exact
RoBERTa numerical settings. This version keeps the hard-weighting recipe, split, loss, max length,
and threshold search, while using DeBERTa-safe CUDA/optimizer settings.

Purpose: test whether DeBERTa-v3-base improves contextual modeling for hidden/deviant AIS behavior
without changing the training strategy.
...
- CUDA safety: TF32 disabled; flash/mem-efficient SDP disabled where available
- Fixed threshold submission: `{SUBMISSION_THRESHOLD}`
- Hard weights:
  - clean TRUE: `2.5`
...
- After every epoch, threshold is tuned over `{THRESHOLD_GRID.min():.2f}`-`{THRESHOLD_GRID.max():.2f}` for validation F1
- Best checkpoint selected by threshold-tuned validation F1, not F1 at 0.5
- Final tuned-threshold submission uses the best epoch threshold; a separate fixed `{SUBMISSION_THRESHOLD}` submission is also saved
...
- `gpu/outputs/roberta_hard_weighted_gpu_b64_final/roberta_hard_weighted_gpu_b64_submission_thr0p32.csv`, public LB `0.9347079`
