#!/usr/bin/env python3
"""
Train RoBERTa with hard-example sample weighting plus R-Drop.
...
The script keeps the proven roberta-base hard-weighted setup, then adds R-Drop:
the same batch is passed through the model twice with dropout enabled and a
symmetric KL consistency loss is added between the two output distributions.

This is a targeted regularization experiment, not a brute-force sweep. The main
submission is generated with threshold 0.32 because that threshold produced the
best public LB for the hard-weighted GPU run.
"""
...
OUTPUT_DIR = ROOT_DIR / "outputs" / "roberta_hard_weighted_rdrop_gpu"
BEST_MODEL_DIR = OUTPUT_DIR / "best_model"
BEST_STATE_DICT_PATH = OUTPUT_DIR / "roberta_hard_weighted_rdrop_gpu_best_state_dict.pt"
...
METRICS_PATH = OUTPUT_DIR / "roberta_hard_weighted_rdrop_gpu_metrics.json"
VAL_PRED_PATH = OUTPUT_DIR / "roberta_hard_weighted_rdrop_gpu_val_predictions.csv"
ERROR_ANALYSIS_PATH = OUTPUT_DIR / "roberta_hard_weighted_rdrop_gpu_error_analysis.csv"
TEST_PROB_PATH = OUTPUT_DIR / "roberta_hard_weighted_rdrop_gpu_test_probabilities.csv"
SUBMISSION_PATH = OUTPUT_DIR / "roberta_hard_weighted_rdrop_gpu_submission.csv"
SUBMISSION_THR032_PATH = OUTPUT_DIR / "roberta_hard_weighted_rdrop_gpu_submission_thr0p32.csv"
NOTES_PATH = OUTPUT_DIR / "roberta_hard_weighted_rdrop_gpu_notes.md"
WEIGHT_SUMMARY_PATH = OUTPUT_DIR / "roberta_hard_weighted_rdrop_gpu_weight_summary.csv"
FEATURE_THRESHOLDS_PATH = OUTPUT_DIR / "roberta_hard_weighted_rdrop_gpu_feature_thresholds.json"
...
def weighted_symmetric_kl(
    ...
    per_row_kl = 0.5 * (kl_ab + kl_ba)
    return (per_row_kl * sample_weights).sum() / sample_weights.sum().clamp_min(1e-12)
...
def main() -> None:
    ...
    print(INSTALL_CMD)
    print("\n=== RoBERTa Hard-Weighted R-Drop Training ===")
    ...
    print("Loss: weighted CE + R-Drop symmetric KL consistency")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for step, batch in enumerate(progress, start=1):
            ...
            outputs_a = model(input_ids=input_ids, attention_mask=attention_mask)
            outputs_b = model(input_ids=input_ids, attention_mask=attention_mask)

            per_row_loss_a = F.cross_entropy(outputs_a.logits, labels, reduction="none")
            per_row_loss_b = F.cross_entropy(outputs_b.logits, labels, reduction="none")
            weighted_ce_a = (per_row_loss_a * sample_weights).sum() / sample_weights.sum().clamp_min(1e-12)
            weighted_ce_b = (per_row_loss_b * sample_weights).sum() / sample_weights.sum().clamp_min(1e-12)
            weighted_ce = 0.5 * (weighted_ce_a + weighted_ce_b)
            rdrop_kl = weighted_symmetric_kl(outputs_a.logits, outputs_b.logits, sample_weights)
            loss = weighted_ce + (RDROP_ALPHA * rdrop_kl)
            ...
            running_unweighted_loss += (
                float((0.5 * (per_row_loss_a.mean() + per_row_loss_b.mean())).detach().cpu().item()) * batch_size
            )
            running_rdrop_kl += float(rdrop_kl.detach().cpu().item()) * batch_size
            ...
            progress.set_postfix(
                ...
                rdrop_kl=f"{(running_rdrop_kl / max(seen_examples, 1)):.4f}",
        ...
        train_rdrop_kl = running_rdrop_kl / max(seen_examples, 1)
        ...
        val_metrics_05["train_rdrop_kl"] = float(train_rdrop_kl)
        ...
        print(
            ...
            f"| train_raw_ce={train_unweighted_loss:.6f} | train_rdrop_kl={train_rdrop_kl:.6f} "
            f"| val_loss={val_metrics_05['val_loss']:.6f} "
    ...
    test_pred = (test_prob_true >= SUBMISSION_THRESHOLD).astype(int)
    ...
    submission_df.to_csv(SUBMISSION_THR032_PATH, index=False)
    print(f"Saved threshold 0.32 submission copy: {SUBMISSION_THR032_PATH}")
    ...
    metrics_payload = {
        ...
        "config": {
            ...
            "loss": "sample_weighted_cross_entropy_plus_rdrop_symmetric_kl",
            "weight_strategy": {
                "hard_clean_TRUE": 2.5,
        ...
        "submission_threshold": {
            "value": float(SUBMISSION_THRESHOLD),
            "reason": "fixed to the public-LB best threshold from the hard-weighted GPU run",
        },
    ...
    notes_text = f"""# RoBERTa Hard-Weighted R-Drop Notes

## Why This Run Exists

The current RoBERTa baseline already captures the easy distribution artifact: noisy TRUE and clean FALSE.
The plateau is mostly clean-looking TRUE and messy-looking FALSE. The best GPU hard-weighted run improved
public LB to `0.9347079` with threshold `0.32`, but later epochs were locally overconfident.

This script keeps the same hard-weighted `roberta-base` idea and adds R-Drop regularization. Each training
batch is passed through the model twice with dropout enabled, then a symmetric KL consistency loss is added.
The goal is to preserve the hard-weighted signal while reducing unstable overconfidence.
...
- Loss: `sample_weighted_cross_entropy + {RDROP_ALPHA} * symmetric_kl`
- R-Drop alpha: `{RDROP_ALPHA}`
...
- Main test submission is still generated at fixed threshold `{SUBMISSION_THRESHOLD}`, because that was the
  best public-LB threshold for the hard-weighted GPU run.
...
Main practical comparison target is the hard-weighted GPU run:
