#!/usr/bin/env python3
"""
Train RoBERTa with model-based label cleaning plus hard-example weighting.

Dependency install command:
pip install pandas numpy scikit-learn tqdm torch transformers accelerate

This script keeps the proven roberta-base hard-weighted recipe, then adds one
data-centric change: use the saved best hard-weighted model as a teacher to
find high-confidence train-label conflicts.

It does not relabel rows. It downweights rows where the teacher strongly
disagrees with the provided label, then retrains the same hard-weighted model.
"""
...
SEVERE_TRUE_CONFLICT_MAX_PROB = 0.05
MODERATE_TRUE_CONFLICT_MAX_PROB = 0.10
MODERATE_FALSE_CONFLICT_MIN_PROB = 0.90
SEVERE_FALSE_CONFLICT_MIN_PROB = 0.95
SEVERE_CONFLICT_MULTIPLIER = 0.25
MODERATE_CONFLICT_MULTIPLIER = 0.55
TEACHER_INFERENCE_BATCH_SIZE = 64

ROOT_DIR = Path(__file__).resolve().parents[1]
DATA_DIR = ROOT_DIR / "data"
TEACHER_MODEL_CANDIDATES = [
    ROOT_DIR / "outputs" / "roberta_hard_weighted_gpu_b64_final" / "best_model",
    ROOT_DIR / "outputs" / "roberta_hard_weighted_gpu_b64_final2" / "best_model",
    ROOT_DIR / "outputs" / "roberta_hard_weighted_gpu_b64" / "best_model",
]

OUTPUT_DIR = ROOT_DIR / "outputs" / "roberta_model_cleaned_hard_weighted_gpu"
BEST_MODEL_DIR = OUTPUT_DIR / "best_model"
BEST_STATE_DICT_PATH = OUTPUT_DIR / "roberta_model_cleaned_hard_weighted_gpu_best_state_dict.pt"
...
METRICS_PATH = OUTPUT_DIR / "roberta_model_cleaned_hard_weighted_gpu_metrics.json"
VAL_PRED_PATH = OUTPUT_DIR / "roberta_model_cleaned_hard_weighted_gpu_val_predictions.csv"
ERROR_ANALYSIS_PATH = OUTPUT_DIR / "roberta_model_cleaned_hard_weighted_gpu_error_analysis.csv"
TEST_PROB_PATH = OUTPUT_DIR / "roberta_model_cleaned_hard_weighted_gpu_test_probabilities.csv"
SUBMISSION_PATH = OUTPUT_DIR / "roberta_model_cleaned_hard_weighted_gpu_submission.csv"
SUBMISSION_THR032_PATH = OUTPUT_DIR / "roberta_model_cleaned_hard_weighted_gpu_submission_thr0p32.csv"
NOTES_PATH = OUTPUT_DIR / "roberta_model_cleaned_hard_weighted_gpu_notes.md"
WEIGHT_SUMMARY_PATH = OUTPUT_DIR / "roberta_model_cleaned_hard_weighted_gpu_weight_summary.csv"
FEATURE_THRESHOLDS_PATH = OUTPUT_DIR / "roberta_model_cleaned_hard_weighted_gpu_feature_thresholds.json"
TEACHER_TRAIN_PROB_PATH = OUTPUT_DIR / "roberta_model_cleaned_hard_weighted_gpu_teacher_train_probabilities.csv"
LABEL_CLEANING_DIAGNOSTICS_PATH = OUTPUT_DIR / "roberta_model_cleaned_hard_weighted_gpu_label_cleaning_diagnostics.csv"
...
def resolve_teacher_model_dir() -> Path:
    for candidate in TEACHER_MODEL_CANDIDATES:
        if (candidate / "config.json").exists():
            return candidate
    candidates_text = "\n".join(f"- {candidate}" for candidate in TEACHER_MODEL_CANDIDATES)
    raise FileNotFoundError(
        "Could not find a saved teacher model checkpoint. Checked:\n"
        f"{candidates_text}\n"
        "Run or sync the best hard-weighted RoBERTa output first."
    )
...
def get_teacher_train_probabilities(
    texts: list[str],
    labels: np.ndarray,
    device: torch.device,
) -> tuple[np.ndarray, Path]:
    if TEACHER_TRAIN_PROB_PATH.exists():
        cached_df = pd.read_csv(TEACHER_TRAIN_PROB_PATH)
        required_cols = {"text", "label", "teacher_prob_TRUE"}
        missing_cols = sorted(required_cols - set(cached_df.columns))
        if missing_cols:
            raise ValueError(f"Cached teacher probability file missing columns: {missing_cols}")
        cached_texts = [normalize_text(value) for value in cached_df["text"]]
        if len(cached_df) == len(texts) and cached_texts == texts:
            teacher_model_dir = resolve_teacher_model_dir()
            print(f"Loaded cached teacher train probabilities: {TEACHER_TRAIN_PROB_PATH}")
            return cached_df["teacher_prob_TRUE"].to_numpy(dtype=np.float32), teacher_model_dir
        print("Cached teacher train probabilities did not align with current train text. Recomputing.")

    teacher_model_dir = resolve_teacher_model_dir()
    print(f"\n=== Teacher scoring for model-based label cleaning ===")
    print(f"Teacher model: {teacher_model_dir}")
    teacher_tokenizer = AutoTokenizer.from_pretrained(teacher_model_dir, use_fast=True)
    teacher_model = AutoModelForSequenceClassification.from_pretrained(teacher_model_dir)
    teacher_model.to(device)

    teacher_input_ids, teacher_attention_mask = tokenize_texts(
        texts,
        teacher_tokenizer,
        max_length=MAX_LENGTH,
        batch_size=256,
        desc="Tokenizing full train for teacher scoring",
    )
    teacher_dataset = TensorDataset(teacher_input_ids, teacher_attention_mask)
    ...
    _, teacher_prob_true, _ = evaluate_model(
        model=teacher_model,
        dataloader=teacher_loader,
        device=device,
        desc="Teacher train inference",
        has_labels=False,
    )
    teacher_prob_true = teacher_prob_true.astype(np.float32)
    teacher_df = pd.DataFrame(
        {
            "row_id": np.arange(len(texts), dtype=int),
            "text": texts,
            "label": decode_labels(labels),
            "teacher_prob_TRUE": teacher_prob_true,
        }
    )
    teacher_df.to_csv(TEACHER_TRAIN_PROB_PATH, index=False)
    print(f"Saved teacher train probabilities: {TEACHER_TRAIN_PROB_PATH}")

    del teacher_model
    del teacher_tokenizer
    if device.type == "cuda":
        torch.cuda.empty_cache()
    return teacher_prob_true, teacher_model_dir


def build_label_cleaning_frame(
    labels: np.ndarray,
    teacher_prob_true: np.ndarray,
) -> pd.DataFrame:
    rows = []
    for label, prob_true in zip(labels, teacher_prob_true, strict=True):
        label = int(label)
        prob_true = float(prob_true)
        if label == 1:
            support_prob = prob_true
            if prob_true <= SEVERE_TRUE_CONFLICT_MAX_PROB:
                multiplier = SEVERE_CONFLICT_MULTIPLIER
                category = "severe_TRUE_label_teacher_FALSE"
            elif prob_true <= MODERATE_TRUE_CONFLICT_MAX_PROB:
                multiplier = MODERATE_CONFLICT_MULTIPLIER
                category = "moderate_TRUE_label_teacher_FALSE"
            else:
                multiplier = 1.0
                category = "teacher_supported_TRUE"
        else:
            support_prob = 1.0 - prob_true
            if prob_true >= SEVERE_FALSE_CONFLICT_MIN_PROB:
                multiplier = SEVERE_CONFLICT_MULTIPLIER
                category = "severe_FALSE_label_teacher_TRUE"
            elif prob_true >= MODERATE_FALSE_CONFLICT_MIN_PROB:
                multiplier = MODERATE_CONFLICT_MULTIPLIER
                category = "moderate_FALSE_label_teacher_TRUE"
            else:
                multiplier = 1.0
                category = "teacher_supported_FALSE"
        rows.append(
            {
                "teacher_prob_TRUE": prob_true,
                "teacher_label": "TRUE" if prob_true >= 0.5 else "FALSE",
                "teacher_label_support_prob": float(support_prob),
                "label_cleaning_multiplier": float(multiplier),
                "label_cleaning_category": category,
                "likely_label_conflict": bool(multiplier < 1.0),
            }
        )
    return pd.DataFrame(rows)
...
def write_weight_summary(
    ...
    grouped = (
        summary_df.groupby(["label", "hard_category", "label_cleaning_category"])
        .agg(
            rows=("label_cleaning_category", "size"),
            mean_abnormal_score=("abnormal_score", "mean"),
            mean_teacher_prob_TRUE=("teacher_prob_TRUE", "mean"),
            mean_label_cleaning_multiplier=("label_cleaning_multiplier", "mean"),
        ...
        .reset_index()
        .sort_values(["label", "hard_category", "label_cleaning_category"])
    )
...
def main() -> None:
    ...
    print(INSTALL_CMD)
    print("\n=== RoBERTa Model-Cleaned Hard-Weighted Training ===")
    ...
    print("Loss: hard-example weighted CE with teacher-based label-conflict downweighting")
    ...
    teacher_prob_true, teacher_model_dir = get_teacher_train_probabilities(
        texts=texts,
        labels=y,
        device=device,
    )
    label_cleaning_df = build_label_cleaning_frame(y, teacher_prob_true)
    label_cleaning_diagnostics_df = pd.DataFrame(
        {
            "row_id": np.arange(len(texts), dtype=int),
            "text": texts,
            "label": decode_labels(y),
            "teacher_prob_TRUE": label_cleaning_df["teacher_prob_TRUE"].to_numpy(),
            "teacher_label": label_cleaning_df["teacher_label"].to_numpy(),
            "teacher_label_support_prob": label_cleaning_df["teacher_label_support_prob"].to_numpy(),
            "label_cleaning_multiplier": label_cleaning_df["label_cleaning_multiplier"].to_numpy(),
            "label_cleaning_category": label_cleaning_df["label_cleaning_category"].to_numpy(),
            "likely_label_conflict": label_cleaning_df["likely_label_conflict"].to_numpy(),
        }
    )
    label_cleaning_diagnostics_df.to_csv(LABEL_CLEANING_DIAGNOSTICS_PATH, index=False)
    print(f"Saved label-cleaning diagnostics: {LABEL_CLEANING_DIAGNOSTICS_PATH}")
    conflict_summary = (
        label_cleaning_diagnostics_df.groupby(["label", "label_cleaning_category"])
        .size()
        .reset_index(name="rows")
        .sort_values(["label", "label_cleaning_category"])
    )
    print("Label-cleaning category counts:")
    ...
    train_cleaning_df = label_cleaning_df.iloc[train_idx].reset_index(drop=True)
    val_cleaning_df = label_cleaning_df.iloc[val_idx].reset_index(drop=True)
    train_weight_df["base_raw_sample_weight"] = train_weight_df["raw_sample_weight"].to_numpy(dtype=np.float32)
    for column in [
        "teacher_prob_TRUE",
        "teacher_label",
        "teacher_label_support_prob",
        "label_cleaning_multiplier",
        "label_cleaning_category",
        "likely_label_conflict",
    ]:
        train_weight_df[column] = train_cleaning_df[column].to_numpy()
        val_weight_df[column] = val_cleaning_df[column].to_numpy()
    train_weight_df["raw_sample_weight"] = (
        train_weight_df["base_raw_sample_weight"].to_numpy(dtype=np.float32)
        * train_weight_df["label_cleaning_multiplier"].to_numpy(dtype=np.float32)
    )
    ...
    train_conflict_count = int(train_weight_df["likely_label_conflict"].sum())
    print(f"Train split label-conflict rows downweighted: {train_conflict_count}")
    ...
    val_pred_df = pd.DataFrame(
        {
            ...
            "teacher_prob_TRUE": val_weight_df["teacher_prob_TRUE"].to_numpy(),
            "teacher_label": val_weight_df["teacher_label"].to_numpy(),
            "label_cleaning_category": val_weight_df["label_cleaning_category"].to_numpy(),
            "label_cleaning_multiplier_if_trained": val_weight_df["label_cleaning_multiplier"].to_numpy(),
            "likely_label_conflict": val_weight_df["likely_label_conflict"].to_numpy(),
    ...
    error_df = error_df[
        [
            ...
            "teacher_label",
            "label_cleaning_category",
            "label_cleaning_multiplier_if_trained",
            "likely_label_conflict",
    ...
    fixed_test_pred = (test_prob_true >= FIXED_LB_THRESHOLD).astype(int)
    ...
    print(f"Saved fixed-threshold submission ({FIXED_LB_THRESHOLD:.2f}): {SUBMISSION_THR032_PATH}")
    print(f"Fixed-threshold test prediction distribution: {fixed_distribution}")
    ...
    metrics_payload = {
        ...
        "config": {
            ...
            "label_cleaning_strategy": {
                "type": "teacher_conflict_downweighting",
                "severe_TRUE_conflict": f"label TRUE and teacher_prob_TRUE <= {SEVERE_TRUE_CONFLICT_MAX_PROB}",
                "moderate_TRUE_conflict": f"label TRUE and teacher_prob_TRUE <= {MODERATE_TRUE_CONFLICT_MAX_PROB}",
                "moderate_FALSE_conflict": f"label FALSE and teacher_prob_TRUE >= {MODERATE_FALSE_CONFLICT_MIN_PROB}",
                "severe_FALSE_conflict": f"label FALSE and teacher_prob_TRUE >= {SEVERE_FALSE_CONFLICT_MIN_PROB}",
                "severe_multiplier": SEVERE_CONFLICT_MULTIPLIER,
                "moderate_multiplier": MODERATE_CONFLICT_MULTIPLIER,
                "train_conflict_rows_downweighted": train_conflict_count,
                "policy": "downweight only; do not delete or relabel rows",
            },
            "feature_thresholds": thresholds,
        },
        "split": {
        ...
        "fixed_threshold": {
            "threshold": FIXED_LB_THRESHOLD,
            "test_prediction_distribution": fixed_distribution,
            "submission": str(SUBMISSION_THR032_PATH),
        },
        "paths": {
            ...
            "teacher_train_probabilities": str(TEACHER_TRAIN_PROB_PATH),
            "label_cleaning_diagnostics": str(LABEL_CLEANING_DIAGNOSTICS_PATH),
            "best_model_dir": str(BEST_MODEL_DIR),
            "best_state_dict": str(BEST_STATE_DICT_PATH),
        },
        "comparison_target": {
            "submission": "outputs/roberta_hard_weighted_gpu_b64_final/roberta_hard_weighted_gpu_b64_submission_thr0p32.csv",
            "public_lb": 0.9347079,
        },
    }
    ...
    notes_text = f"""# RoBERTa Model-Cleaned Hard-Weighted Notes

## Why This Run Exists

The best current single model is the hard-weighted RoBERTa GPU run at public LB `0.9347079`.
Most architecture and loss variants have shifted calibration without finding a better boundary.

This run keeps the same hard-weighted RoBERTa recipe, but adds model-based label cleaning:

- load the saved best hard-weighted model as a teacher
- score every training row
- find high-confidence conflicts between the teacher and the provided label
- downweight those rows during retraining
- do not delete rows
- do not relabel rows
...
- Loss: `sample_weighted_cross_entropy`
- Train split label-conflict rows downweighted: `{train_conflict_count}`
- Hard weights:
  - clean TRUE: `2.5`
...
- Label-cleaning rules:
  - TRUE with teacher probability <= `{SEVERE_TRUE_CONFLICT_MAX_PROB}`: multiplier `{SEVERE_CONFLICT_MULTIPLIER}`
  - TRUE with teacher probability <= `{MODERATE_TRUE_CONFLICT_MAX_PROB}`: multiplier `{MODERATE_CONFLICT_MULTIPLIER}`
  - FALSE with teacher probability >= `{SEVERE_FALSE_CONFLICT_MIN_PROB}`: multiplier `{SEVERE_CONFLICT_MULTIPLIER}`
  - FALSE with teacher probability >= `{MODERATE_FALSE_CONFLICT_MIN_PROB}`: multiplier `{MODERATE_CONFLICT_MULTIPLIER}`
...
- Teacher label-cleaning multipliers fitted from saved model predictions on train rows
- Best checkpoint selected by validation F1 at threshold 0.5
- Final threshold tuned over 0.30-0.70 for best validation F1
- Additional fixed threshold submission saved at `{FIXED_LB_THRESHOLD:.2f}`
...
Compare first against `outputs/roberta_hard_weighted_gpu_b64_final/roberta_hard_weighted_gpu_b64_submission_thr0p32.csv`, public LB `0.9347079`.

## Outputs

- `outputs/roberta_model_cleaned_hard_weighted_gpu/roberta_model_cleaned_hard_weighted_gpu_metrics.json`
- `outputs/roberta_model_cleaned_hard_weighted_gpu/roberta_model_cleaned_hard_weighted_gpu_val_predictions.csv`
- `outputs/roberta_model_cleaned_hard_weighted_gpu/roberta_model_cleaned_hard_weighted_gpu_error_analysis.csv`
- `outputs/roberta_model_cleaned_hard_weighted_gpu/roberta_model_cleaned_hard_weighted_gpu_test_probabilities.csv`
- `outputs/roberta_model_cleaned_hard_weighted_gpu/roberta_model_cleaned_hard_weighted_gpu_submission.csv`
- `outputs/roberta_model_cleaned_hard_weighted_gpu/roberta_model_cleaned_hard_weighted_gpu_submission_thr0p32.csv`
- `outputs/roberta_model_cleaned_hard_weighted_gpu/roberta_model_cleaned_hard_weighted_gpu_notes.md`
- `outputs/roberta_model_cleaned_hard_weighted_gpu/roberta_model_cleaned_hard_weighted_gpu_weight_summary.csv`
- `outputs/roberta_model_cleaned_hard_weighted_gpu/roberta_model_cleaned_hard_weighted_gpu_feature_thresholds.json`
- `outputs/roberta_model_cleaned_hard_weighted_gpu/roberta_model_cleaned_hard_weighted_gpu_teacher_train_probabilities.csv`
- `outputs/roberta_model_cleaned_hard_weighted_gpu/roberta_model_cleaned_hard_weighted_gpu_label_cleaning_diagnostics.csv`
- `outputs/roberta_model_cleaned_hard_weighted_gpu/best_model/`
- `outputs/roberta_model_cleaned_hard_weighted_gpu/roberta_model_cleaned_hard_weighted_gpu_best_state_dict.pt`
"""
