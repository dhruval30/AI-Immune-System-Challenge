...
#
# Toxic-prompt RoBERTa direct fine-tuning baseline for GPU.
#
# This is based on models/train_roberta_base.py, not the hard-weighted script.
# It keeps the simple single-split CrossEntropyLoss setup and swaps the base
# checkpoint to Intel/toxic-prompt-roberta.
...
OUTPUT_DIR = ROOT_DIR / "outputs" / "toxic_prompt_roberta_base_gpu"
BEST_MODEL_DIR = OUTPUT_DIR / "best_model"
BEST_STATE_DICT_PATH = OUTPUT_DIR / "toxic_prompt_roberta_base_gpu_best_state_dict.pt"
...
METRICS_PATH = OUTPUT_DIR / "toxic_prompt_roberta_base_gpu_metrics.json"
VAL_PRED_PATH = OUTPUT_DIR / "toxic_prompt_roberta_base_gpu_val_predictions.csv"
ERROR_ANALYSIS_PATH = OUTPUT_DIR / "toxic_prompt_roberta_base_gpu_error_analysis.csv"
TEST_PROB_PATH = OUTPUT_DIR / "toxic_prompt_roberta_base_gpu_test_probabilities.csv"
SUBMISSION_PATH = OUTPUT_DIR / "toxic_prompt_roberta_base_gpu_submission.csv"
FIXED_SUBMISSION_PATH = OUTPUT_DIR / "toxic_prompt_roberta_base_gpu_submission_thr0p32.csv"
NOTES_PATH = OUTPUT_DIR / "toxic_prompt_roberta_base_gpu_notes.md"
...
def detect_device() -> torch.device:
    # GPU script priority: CUDA -> MPS -> CPU
    if torch.cuda.is_available():
...
def main() -> None:
    ...
    print("\n=== Toxic-Prompt RoBERTa Base Fine-Tuning ===")
    ...
    notes_text = f"""# Toxic-Prompt RoBERTa Base Fine-Tuning Notes

## Why This Run Exists

This script is based on `models/train_roberta_base.py`, not the hard-weighted
GPU script. It keeps the simple direct fine-tuning recipe:

- standard CrossEntropyLoss
- stratified single validation split
- max length 256
- 3 epochs
- no hard-example weights

The only major change is the initialization checkpoint:
`Intel/toxic-prompt-roberta` instead of plain `roberta-base`.
