#!/usr/bin/env python3
"""
Train RoBERTa-large with hard-example sample weighting.

Dependency install command:
pip install pandas numpy scikit-learn tqdm torch transformers accelerate

This is a RoBERTa-large version of the hard-weighted GPU recipe that produced
the current strongest public LB family.

It keeps the same hard-example weighting logic, but uses safer large-model
training settings: smaller micro-batch, gradient accumulation, fewer epochs,
and lower learning rate. The original RoBERTa-base hard-weighted script remains
the main reproduction anchor.
"""
...
SEED = 42
LOCAL_MODEL_NAME = "/workspace/gpu/roberta-large"
...
OUTPUT_DIR = ROOT_DIR / "outputs" / "roberta_large_hard_weighted"
BEST_MODEL_DIR = OUTPUT_DIR / "best_model"
BEST_STATE_DICT_PATH = OUTPUT_DIR / "roberta_large_hard_weighted_best_state_dict.pt"
...
METRICS_PATH = OUTPUT_DIR / "roberta_large_hard_weighted_metrics.json"
VAL_PRED_PATH = OUTPUT_DIR / "roberta_large_hard_weighted_val_predictions.csv"
ERROR_ANALYSIS_PATH = OUTPUT_DIR / "roberta_large_hard_weighted_error_analysis.csv"
TEST_PROB_PATH = OUTPUT_DIR / "roberta_large_hard_weighted_test_probabilities.csv"
SUBMISSION_PATH = OUTPUT_DIR / "roberta_large_hard_weighted_submission.csv"
SUBMISSION_THR032_PATH = OUTPUT_DIR / "roberta_large_hard_weighted_submission_thr0p32.csv"
NOTES_PATH = OUTPUT_DIR / "roberta_large_hard_weighted_notes.md"
WEIGHT_SUMMARY_PATH = OUTPUT_DIR / "roberta_large_hard_weighted_weight_summary.csv"
FEATURE_THRESHOLDS_PATH = OUTPUT_DIR / "roberta_large_hard_weighted_feature_thresholds.json"
...
def main() -> None:
    ...
    print(INSTALL_CMD)
    print("\n=== RoBERTa-large Hard-Weighted Training ===")
    ...
    print(f"Local RoBERTa-large path exists: {Path(LOCAL_MODEL_NAME).exists()}")
    ...
    metrics_payload = {
        ...
        "comparison_target": {
            ...
            "rdrop_gpu_public_lb": 0.93542757,
        },
    }
    METRICS_PATH.write_text(json.dumps(metrics_payload, indent=2), encoding="utf-8")
    print(f"Saved metrics: {METRICS_PATH}")

    notes_text = f"""# RoBERTa-large Hard-Weighted Notes

## Why This Run Exists

The current best family is the hard-weighted RoBERTa-base GPU run, with public LB `0.9347079` at threshold `0.32`.
R-Drop improved it slightly to `0.93542757`, while DeBERTa/ELECTRA did not work well in the current setup.

This script tests RoBERTa-large as the most natural backbone upgrade while keeping the hard-weighting idea.
It uses smaller micro-batches, gradient accumulation, fewer epochs, and a lower learning rate than the
RoBERTa-base GPU script because the large model is easier to overfit.
...
- Fixed threshold submission: `{FIXED_SUBMISSION_THRESHOLD}`
...
- `gpu/outputs/roberta_hard_weighted_rdrop_gpu/roberta_hard_weighted_rdrop_gpu_submission_thr0p32.csv`, public LB `0.93542757`
