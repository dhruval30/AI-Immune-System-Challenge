#!/usr/bin/env python3
"""
Train DistilRoBERTa with hard-example sample weighting.
...
This is a smaller-capacity diagnostic clone of the proven roberta-base hard-
weighted setup. The only intended modeling change is the checkpoint:
...
SEED = 42
LOCAL_MODEL_PATH = Path("/workspace/gpu/distilroberta-base")
MODEL_NAME = str(LOCAL_MODEL_PATH) if LOCAL_MODEL_PATH.exists() else "distilroberta-base"
MAX_LENGTH = 384
EPOCHS = 12
TRAIN_BATCH_SIZE = 16
EVAL_BATCH_SIZE = 64
...
OUTPUT_DIR = ROOT_DIR / "outputs" / "distilroberta_hard_weighted_gpu"
BEST_MODEL_DIR = OUTPUT_DIR / "best_model"
BEST_STATE_DICT_PATH = OUTPUT_DIR / "distilroberta_hard_weighted_gpu_best_state_dict.pt"
...
METRICS_PATH = OUTPUT_DIR / "distilroberta_hard_weighted_gpu_metrics.json"
VAL_PRED_PATH = OUTPUT_DIR / "distilroberta_hard_weighted_gpu_val_predictions.csv"
ERROR_ANALYSIS_PATH = OUTPUT_DIR / "distilroberta_hard_weighted_gpu_error_analysis.csv"
TEST_PROB_PATH = OUTPUT_DIR / "distilroberta_hard_weighted_gpu_test_probabilities.csv"
SUBMISSION_PATH = OUTPUT_DIR / "distilroberta_hard_weighted_gpu_submission.csv"
FIXED_SUBMISSION_THRESHOLD = 0.32
FIXED_SUBMISSION_PATH = OUTPUT_DIR / "distilroberta_hard_weighted_gpu_submission_thr0p32.csv"
NOTES_PATH = OUTPUT_DIR / "distilroberta_hard_weighted_gpu_notes.md"
WEIGHT_SUMMARY_PATH = OUTPUT_DIR / "distilroberta_hard_weighted_gpu_weight_summary.csv"
FEATURE_THRESHOLDS_PATH = OUTPUT_DIR / "distilroberta_hard_weighted_gpu_feature_thresholds.json"
...
def main() -> None:
    ...
    print(INSTALL_CMD)
    print("\n=== DistilRoBERTa Hard-Weighted Training ===")
    ...
    notes_text = f"""# DistilRoBERTa Hard-Weighted Notes

## Why This Run Exists

This is a smaller-capacity diagnostic version of the current hard-weighted RoBERTa setup.
The model changes from `roberta-base` to `distilroberta-base`, while the split, hard-weighting
logic, optimizer, scheduler, max length, and thresholding flow are kept aligned with the proven
GPU hard-weighted script.

This is worth testing because the leaderboard plateau may include label noise and overfitting.
If a smaller RoBERTa-family encoder performs competitively, that suggests regularization/capacity
control matters more than larger encoders.
