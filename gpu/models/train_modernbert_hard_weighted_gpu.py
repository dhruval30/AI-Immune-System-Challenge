#!/usr/bin/env python3
"""
Train ModernBERT with hard-example sample weighting.
...
This is a ModernBERT version of the current best hard-weighted GPU setup.
It keeps the same split, hard-example weighting, and thresholding flow, but
initializes from `answerdotai/ModernBERT-base`.

ModernBERT-specific mechanics are based on the PangolinGuard ModernBERT
fine-tuning reference:
- dynamic padding with DataCollatorWithPadding
- explicit label2id/id2label maps
- BF16 autocast on CUDA when supported
- fused AdamW when available
- larger batch / higher LR ModernBERT-style fine-tuning
"""
...
SEED = 42
LOCAL_MODEL_PATH = Path("/workspace/gpu/modernbert-base")
MODEL_NAME = str(LOCAL_MODEL_PATH) if LOCAL_MODEL_PATH.exists() else "answerdotai/ModernBERT-base"
MAX_LENGTH = 512
EPOCHS = 4
TRAIN_BATCH_SIZE = 64
EVAL_BATCH_SIZE = 64
...
OUTPUT_DIR = ROOT_DIR / "outputs" / "modernbert_hard_weighted_gpu"
BEST_MODEL_DIR = OUTPUT_DIR / "best_model"
BEST_STATE_DICT_PATH = OUTPUT_DIR / "modernbert_hard_weighted_gpu_best_state_dict.pt"
...
METRICS_PATH = OUTPUT_DIR / "modernbert_hard_weighted_gpu_metrics.json"
VAL_PRED_PATH = OUTPUT_DIR / "modernbert_hard_weighted_gpu_val_predictions.csv"
ERROR_ANALYSIS_PATH = OUTPUT_DIR / "modernbert_hard_weighted_gpu_error_analysis.csv"
TEST_PROB_PATH = OUTPUT_DIR / "modernbert_hard_weighted_gpu_test_probabilities.csv"
SUBMISSION_PATH = OUTPUT_DIR / "modernbert_hard_weighted_gpu_submission.csv"
FIXED_SUBMISSION_THRESHOLD = 0.32
FIXED_SUBMISSION_PATH = OUTPUT_DIR / "modernbert_hard_weighted_gpu_submission_thr0p32.csv"
NOTES_PATH = OUTPUT_DIR / "modernbert_hard_weighted_gpu_notes.md"
WEIGHT_SUMMARY_PATH = OUTPUT_DIR / "modernbert_hard_weighted_gpu_weight_summary.csv"
FEATURE_THRESHOLDS_PATH = OUTPUT_DIR / "modernbert_hard_weighted_gpu_feature_thresholds.json"
...
class TokenizedTextDataset(Dataset):
    def __init__(
        ...
        self.length = len(next(iter(encodings.values()))) if encodings else 0
    ...
    def __getitem__(self, idx: int) -> dict[str, Any]:
        ...
        if self.sample_weights is not None:
            item["sample_weights"] = float(self.sample_weights[idx])
        return item


def build_collate_fn(tokenizer: AutoTokenizer) -> Any:
    ...
    def collate(features: list[dict[str, Any]]) -> dict[str, torch.Tensor]:
        ...
        sample_weights = (
            [feature.pop("sample_weights") for feature in features] if "sample_weights" in features[0] else None
        )
...
def batch_to_model_inputs(batch: dict[str, torch.Tensor], device: torch.device) -> dict[str, torch.Tensor]:
    inputs = {
...
def use_bf16_cuda(device: torch.device) -> bool:
...
def build_optimizer(model: AutoModelForSequenceClassification, learning_rate: float, adam_eps: float, fused: bool) -> AdamW:
    ...
    kwargs = {"lr": learning_rate, "eps": adam_eps}
    if fused:
        kwargs["fused"] = True
    try:
        return AdamW(optimizer_groups, **kwargs)
    except TypeError:
...
def main() -> None:
    ...
    print(INSTALL_CMD)
    print("\n=== ModernBERT Hard-Weighted Training ===")
    ...
    use_bf16 = use_bf16_cuda(device)
    use_fused_adamw = device.type == "cuda"
    print(f"Using device: {device}")
    print(f"Model: {MODEL_NAME}")
    print(f"ModernBERT dynamic padding: enabled, pad_to_multiple_of={PAD_TO_MULTIPLE_OF}")
    print(f"CUDA bf16 autocast: {'enabled' if use_bf16 else 'disabled'}")
    print(f"Fused AdamW requested: {'yes' if use_fused_adamw else 'no'}")
    ...
    train_dataset = TokenizedTextDataset(train_encodings, labels=y_train, sample_weights=train_weights)
    val_dataset = TokenizedTextDataset(val_encodings, labels=y_val)
    test_dataset = TokenizedTextDataset(test_encodings)
    ...
    optimizer = build_optimizer(model=model, learning_rate=LEARNING_RATE, adam_eps=ADAM_EPS, fused=use_fused_adamw)
    ...
    metrics_payload = {
        ...
        "config": {
            ...
            "adam_eps": ADAM_EPS,
            "optimizer": "AdamW",
            "fused_adamw_requested": use_fused_adamw,
    ...
    notes_text = f"""# ModernBERT Hard-Weighted Notes

## Why This Run Exists

This is a ModernBERT version of the current hard-weighted GPU setup. It keeps
the same split, hard-example weighting, and thresholding flow, but initializes
from `answerdotai/ModernBERT-base` instead of `roberta-base`.

It applies ModernBERT-specific fine-tuning choices from the PangolinGuard
reference: dynamic padding, explicit label maps, CUDA BF16 autocast when
available, fused AdamW when available, a larger batch size, and a higher
ModernBERT-style learning rate.
...
- CUDA BF16 autocast: `{use_bf16}`
- Fused AdamW requested: `{use_fused_adamw}`
- Loss: `sample_weighted_cross_entropy`
